...
if ($_REQUEST['do'] == 'pmuserstats')
{
	...
	while($user = $db->fetch_array($users))
	{
		...
		$cell[] = "
		<select name=\"u$user[userid]\" onchange=\"js_pm_jump($user[userid], '" . $db->escape_string($user['username']) . "');\" tabindex=\"1\" class=\"bginput\">
		...
		</select><input type=\"button\" class=\"button\" value=\"$vbphrase[go]\" onclick=\"js_pm_jump($user[userid], '" . $db->escape_string($user['username']) . "');\" tabindex=\"1\" />\n\t";
		print_cells_row($cell);
	}
	print_table_footer();
