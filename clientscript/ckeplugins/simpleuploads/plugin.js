...
function PickAndSendFile(editor, forImage, caller, callback) {
	...
	if (typeof FormData == 'undefined')
	{
		...
		if (!fnNumber)
		{
			...
			editor._.simpleuploadsFormInitFn = fnInitPicker = CKEDITOR.tools.addFunction( function() {
				window.setTimeout(function() {
					var picker = document.getElementById("simpleUploadsTarget").contentWindow.document.getElementById("fileupload1");
		...
		"<input type='file' name='attachment[]' id='fileupload1'></form>";
	...
	init : function( editor ) {
		...
		editor.on( 'simpleuploads.startUpload' , function(ev) {
			...
			ev.data.extraFields = {
				...
				contenttypeid: editor.config.vbulletin.attachinfo.contenttypeid,
                                securitytoken: editor.config.vbulletin.securitytoken
			};

			var valueStringArr = editor.config.vbulletin.attachinfo.valuestring.split('&');
			for (i=0; i < valueStringArr.length; i++) {
				var pair = valueStringArr[i].split('=');

				if (pair.hasOwnProperty(1))
				{
					ev.data.extraFields[pair[0]] = pair[1];
				}
			};
...
//			data.xhr = reader;
	...
	xhr.onload = function() {
		var response = xhr.responseText.split(' - '),
		    el = editor.document.getById(data.id);
		
		// final update
		updateProgress(editor, data.id, null);
		// Correct the Undo image
		editor.fire( "updateSnapshot");
		
		var fileUrl = (response[1] != 0 ? fetch_ajax_url("attachment.php?attachmentid=" + response[1] + "&stc=1") : null);;
		...
		else
		{
			if (response[0] == 'ok')
			{
				if (response[1] != 0)
				{
					editor.insert_attachment(response[1], response[2]);
				}
			}
			else
			{
				...
				{
					alert(editor.lang.vbulletin.invalidurl);
				}
				return false;
			}
		}
...
function sendBlobFile(data, xhr)
{
	...
	formdata.append( "attachment[]", data.file, data.name );
	// Add extra fields if provided
	if (data.extraFields)
	{
...
function sendBase64File(data, xhr)
{
	...
	req += rn + "Content-Disposition: form-data; name=\"attachment[]\"; filename=\"" + data.name + "\"";
