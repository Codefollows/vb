...
_ishout = function()
{
	...
	this.initiate = function(thisscript, idletime, dobold, doitalic, dounderline, newestbottom, floodtime, shoutheight, refreshspeed)
	{
		...
		this.append_tab('<a href="?" onclick="return InfernoShoutbox.show(\'shoutbox\');">Shoutbox</a>');
		this.append_tab('<a href="?" onclick="return InfernoShoutbox.show(\'activeusers\');">Active Users</a>: <span id="shoutbox_activity">0</span>');
	...
	this.shout_posted = function()
	{
		...
		if (ajax.handler.readyState == 4 && ajax.handler.status == 200)
		{
			...
			{
				InfernoShoutbox.show_notice('You must wait ' + InfernoShoutbox.floodtime + ' seconds between shouts.');
				InfernoShoutbox.posting_shout = false;
			}
			else if(PHP.trim(ajax.handler.responseText) == 'maxchars')
			{
				InfernoShoutbox.show_notice('You have exceeded the maximum characters per shout.');
				InfernoShoutbox.posting_shout = false;
			}
			else if(PHP.trim(ajax.handler.responseText) == 'exploit')
			{
				InfernoShoutbox.show_notice('You won\'t exploit me that way, sucker.');
				InfernoShoutbox.posting_shout = false;
			}
			else if(PHP.trim(ajax.handler.responseText) == 'silenced')
			{
				InfernoShoutbox.show_notice('You aren\'t allowed to shout right now.');
				InfernoShoutbox.posting_shout = false;
			}
			else
			{
				InfernoShoutbox.show_notice('Your shout failed to send. You may have been logged out.');
				InfernoShoutbox.posting_shout = false;
			}
		}
	}
	...
	this.fetch_aop = function()
	{
		...
		this.fetchaop.ajax.send('inferno/shoutbox/aop/aop.php', '');
	}
	...
	this.fetch_shouts = function()
	{
		...
		{
			this.show_notice('You are currently idle in the shoutbox. Click <a href="?" onclick="return InfernoShoutbox.unidle();">here</a> to un-idle yourself.');
	...
	this.open_pm_tab = function(pmid, username)
	{
		...
		// Create the window
		this.append_shout_window(pmid, '/pm ' + parseInt(pmid.split("_")[1]) + ' ; ', '', 'pmonly&pmid=' + pmid.split('_')[1]);
	...
	this.goto_pm_window = function(windowid)
	{
		...
		this.shoutframe.innerHTML = 'Loading...';
		this.shoutframe.style.display = 'block';
	...
	{
		window.location.href = 'inferno.php?' + SESSIONURL + 'do=options';
	}
	
	this.goto_archive = function()
	{
		window.location.href = 'inferno.php?' + SESSIONURL + 'do=archive';
	}

	this.detach = function()
	{
		detachwin = window.open('inferno.php?' + SESSIONURL + 'do=detach', '_ishout_detach');
		try
		{
			detachwin.focus();
		}
