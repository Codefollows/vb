...
abstract class vB_Upgrade_Abstract
{
	...
	protected function get_upgrade_start($version = null)
	{
		...
		if ($gotlog)
		{
			if (!preg_match('/^upgrade_(\w+)\.php$/siU', $log['script'], $reg))
			{
				...
				if (in_array($log['script'], $this->endscripts) OR preg_match('#^\d+((a|b|g|vbs|rc|pl)\d+)?$#si', $log['script']))
				{
					$gotlog = true;
	...
	*/
	...
	{
		if (preg_match('/^(\w+\s+)?(\d+)\.(\d+)\.(\d+)(\s+(a|alpha|b|beta|g|gamma|rc|release candidate|vbs|by vbs|gold|stable|final|pl|patch level)(\s+(\d+))?)?$/siU', $version, $regs))
		{
	...
	*/
	...
	protected function verify_files()
	{
		...
		if (file_exists(DIR . '/includes/md5_sums_vbulletin.php'))
		{
			...
			$md5_sum_versions = array('vbulletin' => '4.2.6 by vBS');
